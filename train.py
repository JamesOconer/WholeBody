...
import transforms
from model import HighResolutionNet,pose_hrnet
...
def main(args):
    ...
    # model = torch.nn.parallel.DataParallel(model,device_ids=[local_rank],output_device=local_rank)
    model_without_ddp = model.module
    ...
    # scaler = torch.cuda.amp.GradScaler() if True else None
...
if __name__ == "__main__":
    ...
    # 训练文件的根目录(coco2017)
    parser.add_argument('--data-path', default='/data/vfan/dataset/coco/', help='dataset')
    # 训练设备类型
    ...
    # 检测目标类别数(不包含背景)
    parser.add_argument('--num-joints', default=133, type=int, help='num_joints(num_keypoints)')
    # 每块GPU上的batch_size
